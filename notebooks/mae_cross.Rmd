...
def show_image(image, title=""):
    mean = torch.tensor(get_rgb_stats("centered")["mean"])
    std = torch.tensor(get_rgb_stats("centered")["std"])
    # image is [H, W, 3]
...
for e in range(epoch_start, epoch_end + 1):
    try:
        net, _, _ = fs_ops.load_mim_checkpoint(
            device,
            network,
            encoder=encoder,
            tag=tag,
    ...
    transform = inference_preset((size, size), get_rgb_stats("centered"), 1.0)
