...
```{python}
(B, N, L) = x.size()  # N = H*W
b1 = net.encoder.block[0].norm1(x)
show(b1)
```

```{python}
num_heads = net.encoder.block[0].attn.num_heads
head_dim = net.encoder.block[0].attn.head_dim

z = b1 @ net.encoder.block[0].attn.qkv.weight.T + net.encoder.block[0].attn.qkv.bias
...
```{python}
z = net.encoder.block[0].attn.proj(z)
show(z)
```
...
```{python}
b2 = net.encoder.block[0].norm2(b1)
show(b2)
```
