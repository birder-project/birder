...
net, class_to_idx, _, _ = fs_ops.load_detection_checkpoint(
    device,
    "faster_rcnn",
    tag=None,
...
```{python}
class_list = ["Background", *class_list_from_class_to_idx(class_to_idx)]
...
```{python}
boxes, scores = net.rpn.filter_proposals(proposals, objectness, images.image_sizes, num_anchors_per_level)
...
img_tensor = reverse_transform(input_tensor)[0]
annotated_img = draw_bounding_boxes(img_tensor, boxes=boxes[0:idx].detach(), labels=labels[0:idx])
transformed_img = F.to_pil_image(annotated_img)
