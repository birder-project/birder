...
```{python}
(net, model_info) = fs_ops.load_model(device, "vitreg4_b16", path="models/vitreg4_b16_mim_300.pt", inference=True)
...
```{python}
(net, model_info) = fs_ops.load_model(
    device, "hiera_abswin_base", path="models/hiera_abswin_base_mim_303.pt", inference=True
)
```
...
```{python}
(net, model_info) = fs_ops.load_model(
    device, "swin_transformer_v2_s", path="models/swin_transformer_v2_s_intermediate-arabian-peninsula.pt", inference=True
)
```
...
```{python}
rope = RoPE(192, temperature=100.0, grid_size=(16, 16))
```
