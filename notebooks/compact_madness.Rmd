...
```{python}
def get_model_name(file_name: str) -> int:
    reparameterized = False
    if file_name.find("reparameterized") != -1:
        reparameterized = True
        file_name = file_name.replace("_reparameterized", "")

    model_name = file_name[: file_name.find(PATTERN) - 1].replace("_", " ")
...
for file_name in compare_results_df["File name"]:
    ...
    model_names.append(get_model_name(file_name))
...
```{python}
benchmark_df = pl.read_csv(settings.RESULTS_DIR.joinpath(f"benchmark_cpu_{PATTERN}.csv"))
compare_results_df = compare_results_df.join(benchmark_df, left_on="Pretrained name", right_on="model_name")
compare_results_df = compare_results_df.with_columns(
...
compare_results_df = compare_results_df.select(
    "Model name",
    pl.all().exclude("File name", "Model name", "Pretrained name", "Parameters (M)", *benchmark_df.columns),
    "Parameters (M)",
    "samples_per_sec",
)
...
compare_results_df = compare_results_df.sort("ms / sample", descending=False)
compare_results_df = compare_results_df.with_columns(pl.col("Accuracy").cum_max().alias("Pareto frontier (cpu-ms)"))
compare_results_df = compare_results_df.sort("Parameters (M)", descending=False)
compare_results_df = compare_results_df.with_columns(pl.col("Accuracy").cum_max().alias("Pareto frontier (p)"))
```
...
frontier = compare_results_df.plot.line(x="Parameters (M)", y="Pareto frontier (p)").mark_line(
...
text = base.mark_text(align="center", baseline="middle", dy=-10).encode(text="Model name")
frontier = compare_results_df.plot.line(x="ms / sample", y="Pareto frontier (cpu-ms)").mark_line(
    interpolate="step-after", color="red", strokeWidth=0.3, strokeDash=(2, 2)
)
