...
```{python}
PATTERN = "arabian-peninsula"
RESULTS_PATH = "arabian-peninsula"
```
...
def get_model_name_and_num_parameters(file_name: str, tag: str = PATTERN) -> tuple[str, str, int, bool, bool]:
    ...
    return (model_name.replace("_", " "), model_type.replace("_", " "), info["num_params"], intermediate, mim, dist)
```
...
for file_name in compare_results_df["File name"]:
    (model_name, model_type, count, intermediate_trn, mim_trn, dist_trn) = get_model_name_and_num_parameters(file_name)
...
combination_df = (
    ...
    .select("device", "batch_size", "compile", "amp", "single_thread")
    .unique()
)
