...
```{python}
def get_num_parameters(file_name: str, pattern: str = PATTERN) -> int:
    file_name = file_name.split("/")[-1]
    num_classes = file_name[file_name.find(pattern) + len(pattern) + 1 :]
    num_classes = int(num_classes.split("_")[0])
    model_name = file_name[: file_name.find(pattern) - 1]
...
for file_name in compare_results_df["File name"]:
    ...
    model_names.append(file_name[: file_name.find(PATTERN) - 1])
```

```{python}
compare_results_df = compare_results_df.with_columns(
    pl.Series("Parameters (M)", num_param), pl.Series("Model name", model_names)
)
...
base = compare_results_df.plot.point(
    x="Parameters (M)", y="Accuracy", color="Model name"
)
text = base.mark_text(align="left", baseline="middle", dx=7).encode(text="Model name")
chart = base + text
chart.properties(width=600, height=400).configure_scale(zero=False)
```
