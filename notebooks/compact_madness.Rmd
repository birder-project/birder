...
```{python}
# Parameter count
base = param_compare_results_df.plot.point(
    x="Parameters (M)",
    y="Accuracy",
    color="Model name",
    shape="Resolution:N",
...
frontier = param_compare_results_df.plot.line(x="Parameters (M)", y="Pareto frontier (p)").mark_line(
    interpolate="step-after", color="red", strokeWidth=0.3, strokeDash=(2, 2)
)

chart = base + text + frontier
chart.properties(title="Accuray vs Parameter Count", width=960, height=640).configure_scale(zero=False)
```

```{python}
chart.layer[0].encoding.x.scale = alt.Scale(domain=[0.5, 7])
chart.properties(title="Accuray vs Parameter Count", width=960, height=640).configure_scale(zero=False)
```

```{python}
# CPU rate
base = cpu_compare_results_df.plot.point(
    x="ms / sample",
    y="Accuracy",
    color="Model name",
    shape="Resolution:N",
...
text = base.mark_text(align="center", baseline="middle", dy=-10).encode(text="Model name")
frontier = cpu_compare_results_df.plot.line(x="ms / sample", y="Pareto frontier (cpu-ms)").mark_line(
    interpolate="step-after", color="red", strokeWidth=0.3, strokeDash=(2, 2)
)

chart = base + text + frontier
chart.properties(title="Accuray vs CPU Rate", width=960, height=640).configure_scale(zero=False)
```

```{python}
# CUDA rate
base = cuda_compare_results_df.plot.point(
    x="ms / sample",
    y="Accuracy",
    color="Model name",
    shape="Resolution:N",
...
text = base.mark_text(align="center", baseline="middle", dy=-10).encode(text="Model name")
frontier = cuda_compare_results_df.plot.line(x="ms / sample", y="Pareto frontier (cuda-ms)").mark_line(
    interpolate="step-after", color="red", strokeWidth=0.3, strokeDash=(2, 2)
)

chart = base + text + frontier
chart.properties(title="Accuray vs CUDA Rate", width=960, height=640).configure_scale(zero=False)
```
