...
def get_num_parameters(file_name: str, tag: str = PATTERN) -> int:
    ...
    if reparameterize_available(model) is True:
...
```{python}
compare_results_df = compare_results_df.with_columns(
    pl.col("Accuracy").cum_max().alias("Pareto frontier")
)
```
...
frontier = compare_results_df.plot.line(
    x="Parameters (M)", y="Pareto frontier"
).mark_line(interpolate="step-after", color="red", strokeWidth=0.25, strokeDash=(2, 2))
