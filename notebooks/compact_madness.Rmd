...
```{python}
results_dict = {}
for result_file in settings.RESULTS_DIR.joinpath("il-common-v1").glob(f"*{PATTERN}*"):
...
compare_results_df = compare_results_df.select("Model name", pl.all().exclude("File name", "Model name"))
compare_results_df = compare_results_df.sort("Parameters (M)", descending=False)
```

```{python}
compare_results_df = compare_results_df.with_columns(pl.col("Accuracy").cum_max().alias("Pareto frontier"))
```
...
frontier = compare_results_df.plot.line(x="Parameters (M)", y="Pareto frontier").mark_line(
    interpolate="step-after", color="red", strokeWidth=0.25, strokeDash=(2, 2)
)
