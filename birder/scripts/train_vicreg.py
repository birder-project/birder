...
def train(args: argparse.Namespace, overrides: Optional[TrainOverrides] = None) -> None:
    ...
    if len(set(naflex_specs)) > 1:
        ...
        naflex_transforms = {
            spec: TrainTransform(training_utils.get_naflex_training_transform(args, spec)) for spec in naflex_specs
        }
        ...
        spec = naflex_specs[0]
        training_transform = TrainTransform(training_utils.get_naflex_training_transform(args, spec))
    else:
    ...
    for epoch in range(begin_epoch, args.stop_epoch):
        ...
        for i, (_, (x, y), _) in batch_iter:
            ...
            if args.naflex is True:
                x, grid_sizes1, valid_mask1 = x
                y, grid_sizes2, valid_mask2 = y
                batch_kwargs = {
                    "grid_sizes1": grid_sizes1.to(device, non_blocking=True),
                    "valid_mask1": valid_mask1.to(device, non_blocking=True),
                    "grid_sizes2": grid_sizes2.to(device, non_blocking=True),
            ...
            with sync_context():
                with torch.amp.autocast(device.type, enabled=args.amp, dtype=amp_dtype):
                    if return_moe_training_output is True:
                        vicreg_loss, moe_training_output = net(x, y, return_moe_training_output=True, **batch_kwargs)
                        raw_loss = vicreg_loss
                        ...
                        if moe_spec.has_auxiliary_loss is True:  # type: ignore[union-attr]
                            ...
                            raw_loss = vicreg_loss + moe_aux_loss
                    else:
                        vicreg_loss = net(x, y, **batch_kwargs)
                        raw_loss = vicreg_loss
            ...
            # Statistics
            running_loss.update(vicreg_loss.detach())
            if running_moe_aux_loss is not None:
