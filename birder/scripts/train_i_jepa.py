...
class TrainCollator:
    ...
    def __call__(self, batch: Any) -> tuple[Any, list[torch.Tensor], list[torch.Tensor]]:
        B = len(batch)
...
def train(args: argparse.Namespace, overrides: Optional[TrainOverrides] = None) -> None:
    ...
    if args.grad_checkpointing is True:
        encoder.backbone.set_grad_checkpointing(
    ...
    momentum_schedule = training_utils.cosine_scheduler(0.996, 1.0, args.epochs, 0, epoch_num_batches)
    ...
    for epoch in range(begin_epoch, args.stop_epoch):
        ...
        for i, ((_, images, _), enc_masks, pred_masks) in batch_iter:
