"""
Inspired by the paper "Unsupervised Representation Learning by Predicting Image Rotations",
...
ImageTransform = Callable[[Any], tuple[torch.Tensor, int]]
...
class TrainTransform:
    """
    Applies a given transformation and then rotates the image by one of the specified angles.
    The 0-degree rotation is chosen with probability (1 - prob).
    Any of the non-zero rotations are chosen uniformly with probability 'prob'.
    It is expected that 'angles[0]' corresponds to a 0-degree rotation.
    """

    def __init__(self, transform: Callable[..., torch.Tensor], angles: list[int], prob: float) -> None:
        if angles[0] != 0:
            raise ValueError("Expected angles list to start with 0 for the 0-degree rotation")

        self.transform = transform
        self.angles = angles
        self.prob = prob

    def __call__(self, sample: Any) -> tuple[torch.Tensor, int]:
        if torch.rand(1).item() > self.prob:
            angle_idx = 0
        else:
            angle_idx = torch.randint(1, len(self.angles), (1,)).item()
...
def train(args: argparse.Namespace, overrides: Optional[TrainOverrides] = None) -> None:
    ...
    if overrides.training_transform is not None:
        ...
        training_transform = TrainTransform(
            training_utils.get_training_transform(args), angles=angles, prob=args.rotation_prob
        )
    ...
    network_name = get_network_name(args.network, tag="rotnet")
    ...
    for epoch in range(begin_epoch, args.stop_epoch):
        ...
        for i, (_, (inputs, targets), _) in batch_iter:
            ...
            with sync_context():
                with torch.amp.autocast(device.type, enabled=args.amp, dtype=amp_dtype):
                    if return_moe_training_output is True:
                        outputs, moe_training_output = net(inputs, return_moe_training_output=True)
                    else:
                        outputs = net(inputs)

                    rotation_loss = criterion(outputs, targets)
                    raw_loss = rotation_loss
                    if return_moe_training_output is True:
                        ...
                        if moe_spec.has_auxiliary_loss is True:  # type: ignore[union-attr]
                            ...
                            raw_loss = rotation_loss + moe_aux_loss
            ...
            # Statistics
            running_loss.update(rotation_loss.detach())
            if running_moe_aux_loss is not None:
...
def get_args_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Usage examples\n"
            "==============\n"
            "torchrun --nproc_per_node=2 -m birder.scripts.train_rotnet \\\n"
            "    --network regnet_y_4g \\\n"
            "    --lr 0.2 \\\n"
            "    --lr-scale 256 \\\n"
            "    --lr-scheduler cosine \\\n"
