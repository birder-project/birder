"""
Franca, adapted from
https://github.com/valeoai/Franca/blob/main/franca/train/ssl_meta_arch.py
...
from birder.net.ssl.franca import DINOLossMRL
from birder.net.ssl.franca import FrancaStudent
from birder.net.ssl.franca import FrancaTeacher
from birder.net.ssl.franca import iBOTPatchLossMRL
...
class FrancaInverseRollBlockMasking(RollBlockMasking):
    def __call__(self, num_masking_patches: int) -> torch.Tensor:
        ...
        # Sample aspect ratio, constrained by image dimensions
        min_lar = max(self.log_aspect_ratio[0], np.log(complement_patches / (self.width**2)))
        max_lar = min(
            self.log_aspect_ratio[1],
            np.log(self.height**2 / (complement_patches + 1e-5)),
        )
        aspect_ratio = math.exp(random.uniform(min_lar, max_lar))

        # Calculate block dimensions (ceil to ensure >= complement_patches)
        h = int(np.ceil(math.sqrt(complement_patches * aspect_ratio)))
...
def train(args: argparse.Namespace) -> None:
    ...
    backbone_name = get_network_name(args.network, tag="franca")
    ...
    mask_generator = FrancaInverseRollBlockMasking(mask_size, num_masking_patches=0, min_aspect=0.33, max_aspect=3.33)
...
def get_args_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        allow_abbrev=False,
        description="Pre-train model with Franca (Nested Matryoshka Clustering)",
        epilog=(
            "Usage examples\n"
            "==============\n"
            "torchrun --nproc_per_node=2 -m birder.scripts.train_franca \\\n"
            "    --network vit_b14 \\\n"
            "    --ibot-separate-head \\\n"
