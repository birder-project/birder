...
def train(args: argparse.Namespace, overrides: Optional[TrainOverrides] = None) -> None:
    ...
    for epoch in range(begin_epoch, args.stop_epoch):
        ...
        for i, ((_, images, _), ids_keep, predict_indices) in batch_iter:
            ...
            with student_sync_context():
                with torch.amp.autocast(device.type, enabled=args.amp, dtype=amp_dtype):
                    if return_moe_training_output is True:
                        pred, student_global_logits, moe_training_output = student(
                ...
                capi_dino_loss = raw_capi_loss + (args.dino_loss_weight * raw_dino_loss)
                if moe_spec is not None and moe_spec.has_auxiliary_loss is True:
                    raw_loss = capi_dino_loss + moe_aux_loss  # pylint: disable=used-before-assignment
                else:
                    raw_loss = capi_dino_loss
            ...
            # Statistics
            running_loss.update(capi_dino_loss.detach())
