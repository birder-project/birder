...
def train(args: argparse.Namespace) -> None:
    ...
    backbone_name = get_network_name(args.network, tag="lejepa")
    ...
    if args.no_summary is False:
        local_shape = (batch_size, args.channels, *args.local_crop_size)
        summary_input = [torch.rand(sample_shape, device=device, dtype=model_dtype) for _ in range(2)]
        summary_input += [
            torch.rand(local_shape, device=device, dtype=model_dtype) for _ in range(args.local_crops_number)
        ]
        summary = torchinfo.summary(
            net_for_info,
            device=device,
            input_data={"x": summary_input},
            dtypes=[model_dtype],
                ...
                if training_utils.is_global_primary(args) is True:
                    summary_writer.add_scalars(
                        "loss",
                        {
                            ...
                            "sigreg": running_sigreg.avg,
                            "invariance": running_inv.avg,
                        },
...
def get_args_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Usage examples\n"
            "==============\n"
            "torchrun --nproc_per_node=2 -m birder.scripts.train_lejepa \\\n"
            "    --network vit_s16 \\\n"
            "    --batch-size 128 \\\n"
            "    --opt adamw \\\n"
            "    --lr 0.0005 \\\n"
    ...
    parser.add_argument("--projection-dim", type=int, default=128, help="projector output dimension")
    parser.add_argument("--projection-hidden-dim", type=int, default=2048, help="projector hidden dimension")
    parser.add_argument("--projection-layers", type=int, default=2, help="number of hidden projector blocks")
    parser.add_argument("--loss-lambda", type=float, default=0.02, help="SIGReg loss weight")
    parser.add_argument("--num-slices", type=int, default=256, help="SIGReg random projection directions")
    parser.add_argument("--num-knots", type=int, default=17, help="SIGReg quadrature knots")
    parser.add_argument("--t-max", type=float, default=3.0, help="SIGReg integration max bound")
    parser.add_argument("--local-crops-number", type=int, default=6, help="number of local views to generate")
    parser.add_argument(
