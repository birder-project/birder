...
def train(args: argparse.Namespace, overrides: Optional[TrainOverrides] = None) -> None:
    ...
    if args.grad_checkpointing is True:
        if args.network.startswith("crossmae") is True:
            logger.warning(
                "CrossMAE uses a full-feature encoder path that is not checkpointed, "
                "so --grad-checkpointing will not reduce memory for its encoder blocks"
            )

        net.encoder.set_grad_checkpointing(
    ...
    for epoch in range(begin_epoch, args.stop_epoch):
        ...
        for i, (_, inputs, _) in batch_iter:
            ...
            # Statistics
            running_loss.update(mim_loss.detach())
            if running_moe_aux_loss is not None:
