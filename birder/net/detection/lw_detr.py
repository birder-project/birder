...
* Uses proposal-relative bbox reparameterization in two-stage mode and additive
  reference-logit refinement in one-stage mode
* Keeps residual shortcuts in the projector C2f bottlenecks
* Aligns multi-resolution backbone features before projection to support generic backbones
...
class Bottleneck(nn.Module):
    def __init__(self, channels: int, expansion: float = 1.0, shortcut: bool = True) -> None:
        super().__init__()
...
class C2f(nn.Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, num_blocks: int, expansion: float = 0.5, shortcut: bool = True
    ) -> None:
        super().__init__()
        ...
        self.blocks = nn.ModuleList([Bottleneck(hidden, expansion=1.0, shortcut=shortcut) for _ in range(num_blocks)])
        self.conv2 = Conv2dNormActivation(
            hidden * (2 + num_blocks),
            out_channels,
            kernel_size=(1, 1),
...
class MultiScaleProjector(nn.Module):
    ...
    def forward(self, features: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        for stage_idx, stage in enumerate(self.stages):
            ...
            target_h = math.ceil(ref_h * scale)
            target_w = math.ceil(ref_w * scale)
...
class LW_DETRDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.two_stage is True:
            ...
            reference_point_logits = reference_points  # For TorchScript compatibility
        else:
            if self.training is True:
            ...
            reference_point_logits = refpoint_embed_weight.unsqueeze(0).expand(memory.size(0), -1, -1)

            # One-stage mode follows the additive-logit parameterization: attention
            # consumes normalized references while box deltas are added to the logits.
            reference_points = reference_point_logits.sigmoid()
        ...
        query_pos = self.query_pos_head(
            _gen_sine_embed_for_position(reference_points_input[:, :, 0], self.hidden_dim // 2).to(target.dtype)
        )
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if return_intermediates is True or layer_idx == last_idx:
                ...
                if self.two_stage is True:
                    ...
                    layer_bboxes = (bbox_delta + reference_point_logits).sigmoid()
...
class LW_DETR(DetectionBaseNet):
    ...
    def __init__(
        ...
        *,
        ...
        self.matcher = HungarianMatcher(
            class_weight=self.match_cost_class, bbox_weight=self.loss_bbox, giou_weight=self.loss_giou
        )
    ...
    def _match_with_groups(
        ...
        if self.group_detr <= 1:
            return self.matcher.match(cls_logits, box_output, targets)

        batch_size = cls_logits.size(0)
        group_detr = self.group_detr
        total_queries = cls_logits.size(1)
        # torch._assert(total_queries % group_detr == 0)

        num_queries_per_group = total_queries // group_detr
        grouped_logits = cls_logits.reshape(batch_size, group_detr, num_queries_per_group, cls_logits.size(-1))
        grouped_boxes = box_output.reshape(batch_size, group_detr, num_queries_per_group, box_output.size(-1))
        assignments_by_group = self.matcher.match_grouped(grouped_logits, grouped_boxes, targets)

        merged_assignments = []
        for image_assignments in zip(*assignments_by_group):
            prediction_indices = torch.concat(
                [
                    group_indices + group_index * num_queries_per_group
                    for group_index, (group_indices, _target_indices) in enumerate(image_assignments)
                ]
            )
            target_indices = torch.concat([target_indices for _group_indices, target_indices in image_assignments])
            merged_assignments.append((prediction_indices, target_indices))

        return merged_assignments

    def _get_num_boxes(self, targets: list[dict[str, torch.Tensor]], device: torch.device) -> torch.Tensor:
        num_boxes = sum(len(target["labels"]) for target in targets) * self.group_detr
        num_boxes = torch.as_tensor(num_boxes, dtype=torch.float, device=device)
        if training_utils.is_dist_available_and_initialized() is True:
            torch.distributed.all_reduce(num_boxes)

        return torch.clamp(num_boxes / training_utils.get_world_size(), min=1)
    ...
    def _compute_loss_from_outputs(
        ...
        num_boxes = self._get_num_boxes(targets, cls_logits.device)
        ...
        for idx in range(cls_logits.size(0)):
            ...
            loss_ce_list.append(self._class_loss(cls_logits[idx], box_output[idx], targets, indices, num_boxes))
    ...
    def _compute_enc_output_loss(
        ...
        num_boxes = self._get_num_boxes(targets, enc_cls_logits.device)

        indices = self._match_with_groups(enc_cls_logits, enc_box_output, targets)
        loss_ce = self._class_loss(enc_cls_logits, enc_box_output, targets, indices, num_boxes)
        loss_bbox, loss_giou = self._box_loss(enc_box_output, targets, indices, num_boxes)
