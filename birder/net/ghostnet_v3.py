"""
GhostNet v3, adapted from
https://github.com/huggingface/pytorch-image-models/blob/main/timm/models/ghostnet.py

Paper "GhostNetV3: Exploring the Training Strategies for Compact Models", https://arxiv.org/abs/2404.11202
...
class GhostModule_v3(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        use_act: bool,
        mode: Literal["original", "shortcut"],
        reparameterized: bool,
    ) -> None:
        super().__init__()
        self.gate_fn = nn.Sigmoid()
        self.out_channels = out_channels
        self.use_shortcut = mode == "shortcut"
        ...
        if use_act is True:
            self.primary_activation = nn.ReLU(inplace=True)
        else:
            self.primary_activation = nn.Identity()

        self.cheap_operation = MobileOneBlock(
            in_channels=init_channels,
            out_channels=new_channels,
            kernel_size=dw_size[0],
            stride=1,
            padding=(dw_size[0] - 1) // 2,
        ...
        if use_act is True:
            self.cheap_activation = nn.ReLU(inplace=True)
        else:
        ...
        if self.use_shortcut is True:
            ...
            self.short_conv = nn.Identity()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x1 = self.primary_activation(self.primary_conv(x))
        x2 = self.cheap_activation(self.cheap_operation(x1))
...
class GhostBottleneck(nn.Module):
    def __init__(
        ...
        if stride[0] > 1 or stride[1] > 1:
            self.conv_dw = MobileOneBlock(
                in_channels=mid_channels,
                out_channels=mid_channels,
                kernel_size=dw_kernel_size[0],
                stride=stride[0],
                padding=(dw_kernel_size[0] - 1) // 2,
                groups=mid_channels,
                use_se=False,
                use_act=False,
                use_scale_branch=True,
...
class GhostNet_v3(DetectorBackbone):
    def __init__(
        self,
        input_channels: int,
        num_classes: int,
        *,
        ...
        for cfg in block_config:
            for k, exp_size, c, se_ratio, s in cfg:
                ...
                if layer_idx > 1:
                    mode: Literal["original", "shortcut"] = "shortcut"
                else:
                    mode = "original"
...
registry.register_model_config("ghostnet_v3_0_5", GhostNet_v3, config={"width": 0.5})
registry.register_model_config("ghostnet_v3_1_0", GhostNet_v3, config={"width": 1.0})
registry.register_model_config("ghostnet_v3_1_3", GhostNet_v3, config={"width": 1.3})
registry.register_model_config("ghostnet_v3_1_6", GhostNet_v3, config={"width": 1.6})
