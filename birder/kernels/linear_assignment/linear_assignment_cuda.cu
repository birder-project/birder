...
augmenting_path_cuda(int nr, int nc, int i, const scalar_t *cost, int64_t cost_row_stride, int64_t cost_col_stride,
                     scalar_t *u, scalar_t *v, int *path, int *row4col, scalar_t *shortestPathCosts, uint8_t *SR,
                     uint8_t *SC, int *remaining, scalar_t *p_minVal, scalar_t infinity) {
    scalar_t minVal = 0;
...
template <typename scalar_t>
__device__ __forceinline__ void solve_cuda_kernel(int nr, int nc, const scalar_t *cost, int64_t cost_row_stride,
                                                  int64_t cost_col_stride, scalar_t *u, scalar_t *v,
                                                  scalar_t *shortestPathCosts, int *path, int *col4row, int *row4col,
                                                  uint8_t *SR, uint8_t *SC, int *remaining, scalar_t infinity) {
    scalar_t minVal;
    for (int cur_row = 0; cur_row < nr; ++cur_row) {
        int sink = augmenting_path_cuda(nr, nc, cur_row, cost, cost_row_stride, cost_col_stride, u, v, path, row4col,
                                        shortestPathCosts, SR, SC, remaining, &minVal, infinity);
...
template <typename scalar_t>
__global__ void solve_cuda_kernel_batch(int bs, int nr, int nc, const scalar_t *cost, int64_t cost_batch_stride,
                                        int64_t cost_row_stride, int64_t cost_col_stride, scalar_t *u, scalar_t *v,
                                        scalar_t *shortestPathCosts, int *path, int *col4row, int *row4col, uint8_t *SR,
                                        uint8_t *SC, int *remaining, scalar_t infinity) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i >= bs) {
        return;
    }

    solve_cuda_kernel(nr, nc, cost + static_cast<int64_t>(i) * cost_batch_stride, cost_row_stride, cost_col_stride,
                      u + i * nr, v + i * nc, shortestPathCosts + i * nc, path + i * nc, col4row + i * nr,
                      row4col + i * nc, SR + i * nr, SC + i * nc, remaining + i * nc, infinity);
}

template <typename scalar_t>
void solve_cuda_batch(c10::ScalarType scalar_type, int device_index, int bs, int nr, int nc, const scalar_t *cost,
                      int64_t cost_batch_stride, int64_t cost_row_stride, int64_t cost_col_stride, int *col4row,
                      int *row4col) {
    ...
    solve_cuda_kernel_batch<<<grid_size, block_size, 0, stream.stream()>>>(
        bs, nr, nc, cost, cost_batch_stride, cost_row_stride, cost_col_stride, u.data_ptr<scalar_t>(),
        v.data_ptr<scalar_t>(), shortest_path_costs.data_ptr<scalar_t>(), path.data_ptr<int>(), col4row, row4col,
        SR.data_ptr<uint8_t>(), SC.data_ptr<uint8_t>(), remaining.data_ptr<int>(), infinity);
...
std::vector<torch::Tensor> batch_linear_assignment_cuda_impl(const torch::Tensor &cost, bool transpose_cost) {
    ...
    AT_DISPATCH_FLOATING_TYPES(cost.scalar_type(), "solve_cuda_batch", [&] {
        solve_cuda_batch<scalar_t>(cost.scalar_type(), device.index(), batch_size, nr, nc, cost.data_ptr<scalar_t>(),
                                   cost_batch_stride, cost_row_stride, cost_col_stride, col4row.data_ptr<int>(),
