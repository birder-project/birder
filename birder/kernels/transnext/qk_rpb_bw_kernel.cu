...
__global__ void
rpb_bw_kernel(const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_attn_weight, // B,H,L,span
              torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> d_rpb, int height, int width,
              int kernel_size, int d_rpb_numel) {
...
template <typename scalar_t>
__global__ void qk_bw_kernel(const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_attn_weight,
                             const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> keys,
                             torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_queries, int height,
...
template <typename scalar_t>
__global__ void
qk_inverse_bw_kernel(const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_attn_weight,
                     const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> queries,
                     torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_keys, int height, int width,
                     int kernel_size) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
...
std::vector<torch::Tensor> qk_rpb_bw_cu(const torch::Tensor d_attn_weight, const torch::Tensor queries,
                                        ...
                                        int cuda_threads) {
    TORCH_CHECK((cuda_threads > 0) && (cuda_threads <= 1024),
                "The value of CUDA_NUM_THREADS should between 1 and 1024");
    ...
    const dim3 rpb_blocks((N + rpb_threads.x - 1) / rpb_threads.x, (L + rpb_threads.y - 1) / rpb_threads.y,
                          (attention_span + rpb_threads.z - 1) / rpb_threads.z);

    const dim3 qk_threads(QK_BATCHTHREADS, QK_PIXELTHREADS, QK_DIMTHREADS);
    const dim3 qk_blocks(((B * N) + qk_threads.x - 1) / qk_threads.x, (L + qk_threads.y - 1) / qk_threads.y,
                         (C + qk_threads.z - 1) / qk_threads.z);

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        queries.scalar_type(), "qk_bw_cu", ([&] {
            rpb_bw_kernel<scalar_t>
                <<<rpb_blocks, rpb_threads>>>(d_attn_weight.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                                              d_rpb.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(), height,
                                              width, kernel_size, d_rpb_numel);
            qk_bw_kernel<scalar_t><<<qk_blocks, qk_threads>>>(
                ...
                d_queries.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), height, width, kernel_size);
            qk_inverse_bw_kernel<scalar_t><<<qk_blocks, qk_threads>>>(
                d_attn_weight.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                queries.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                d_keys.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), height, width, kernel_size);
        }));
