...
__global__ void av_bw_kernel(const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_output,
...
template <typename scalar_t>
__global__ void
av_inverse_bw_kernel(const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> attn_weight,
                     const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_output,
                     torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> d_values, int height,
...
std::vector<torch::Tensor> av_bw_cu(const torch::Tensor d_output, const torch::Tensor attn_weight,
    ...
    const dim3 A_blocks(((B * N) + A_threads.x - 1) / A_threads.x, (L + A_threads.y - 1) / A_threads.y,
                        (attention_span + A_threads.z - 1) / A_threads.z);
    ...
    const dim3 V_blocks(((B * N) + V_threads.x - 1) / V_threads.x, (L + V_threads.y - 1) / V_threads.y,
                        (C + V_threads.z - 1) / V_threads.z);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        attn_weight.scalar_type(), "av_bw_cu", ([&] {
            av_bw_kernel<scalar_t><<<A_blocks, A_threads>>>(
                ...
                d_attn_weight.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), height, width, kernel_size);
            av_inverse_bw_kernel<scalar_t><<<V_blocks, V_threads>>>(
                attn_weight.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                d_output.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                d_values.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), height, width, kernel_size);
        }));
