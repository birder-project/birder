...
A ResNeXt 101 image encoder pretrained using SSCD on the DISC dataset for image copy detection, converted to the Birder format for image feature extraction.
This version retains the original model weights.
The model produces 1024-dimensional L2-normalized descriptors for each input image.
